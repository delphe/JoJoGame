...
--:Star Platinum Prime (Ending Saga, Episode 48):--
...
local StandName = "Star Platinum"
local ModelCreator = "No_Intent"
...
local TimeStopSeconds = 5 -- Time Stop Time - In seconds, just normal numbers no .2 stuff
...
function TimeStop()
	...
	Clock.Enabled = true
	Lock(Character)
	local StopAudio = CreateSound(2404992854, Head, 5.5, 1, 0, false, false, "Stop Shout")
	local i = 0.045
	repeat
		game:GetService("RunService").RenderStepped:Wait()
	...
	until StopAudio.TimePosition > 2.39
	TimeStopped = true
...
function StrongPunch()
	...
	local WalkSpeed = Hum.WalkSpeed
	Hum.WalkSpeed = 7
	CreateSound(271120244, CurrentStand.Head, 6.5, math.random(97,108)/100, 0, false, false, "Strong Shout")
...
Mouse.KeyDown:Connect(function(Key)
	...
	if Key == "e" and Move == false then
		BarrageDown = true
		Barrage(30, false, 2404992475, true)
	end
	...
	if Key == "g" then
		...
		CreateSound(3052256550, Head, 3.5, 1, 0.8, false, false, "Yare Yare Daze...")
