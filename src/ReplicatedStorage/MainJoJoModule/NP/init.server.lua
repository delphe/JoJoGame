...
--=(Category = Fun, Action, Quick, Memey)=--
...
local StandName = "Noob Platinum"
local ModelCreator = "XDavodioX"
...
local TimeStopSeconds = 9 -- Time Stop Time - In seconds, just normal numbers no .2 stuff
local TimeStopImmunity = 6 -- Time Stop Immunity when someone else Time Stops - In seconds, just normal numbers no .2 stuff
local Move = false
local BarrageDown = false
local Anim = "Idle"
local TimeStopped = false
local TimeStopHits = {}
local StrongOOF = false
...
function HitEffect(OriginalHit, Character, RootFrame, HitType)
	...
	if HitType == "Barrage" then
		...
		if Humanoid.Health < 20 and Humanoid.Health > 0 then
			Humanoid.Health = Humanoid.Health + 19.5
	...
	HB.Touched:Connect(function(Toucher)
		if IsAHumanoid(Toucher) and IsAHumanoid(Toucher) ~= Character and IsAHumanoid(Toucher):FindFirstChildOfClass("Humanoid").Health > 0 then
			...
			if TimeStopped == false then
				...
				CreateSound(1825260752, Toucher, 5.5, math.random(95,113)/100, 0, false, false, "Hit SFX")
				...
				if Damage == "Instant Kill" then
					...
					CreatePopUpGui(Toucher, "Jump", tostring(Damage*1.2), Color3.fromRGB(188, 0, 0))
				end
				...
				table.insert(TimeStopHits, {Humanoid, Damage*1.2, 1825260752, Effect, EffectColor, CurrentStand.RootPart.CFrame, HitType})
			end
		end
	end)
...
function Stand()
	...
	if CurrentStand == nil then
		...
		CreateSound(1825260752, Head, 8, 1, 0, false, false, "Stand Call")
...
function Barrage(Times, NeutralStop, BarrageSFX)
	...
	repeat
		game:GetService("RunService").RenderStepped:Wait()
		Repeat = Repeat - 1
		CreateSound(BarrageSFX, CurrentStand.Head, 5.25, math.random(98,103)/100, 0, false, false, "Oofer Man")
...
function TimeStop()
	...
	Clock.Enabled = true
	Lock(Character)
	local StopAudio = CreateSound(1825260752, CurrentStand.Head, 8.5, 1, 0, false, false, "Stop Shout")
	repeat
		game:GetService("RunService").RenderStepped:Wait()
	...
	until StopAudio.TimePosition > 0.39
	TimeStopped = true
	StopAudio:Destroy()
	CreateSound(2152227673, Torso, 6.5, 1, 0.06, false, false, "Time Stop")
...
function StrongPunch()
	...
	local WalkSpeed = Hum.WalkSpeed
	Hum.WalkSpeed = 7
	CreateSound(1825260752, CurrentStand.Head, 6.5, math.random(97,108)/100, 0, false, false, "Strong Shout")
	for i = 0,1,0.31*AttackSpeed/0.75 do
		game:GetService("RunService").RenderStepped:Wait()
		CurrentStand.RootPart["Root Joint"].C0 = CurrentStand.RootPart["Root Joint"].C0:Lerp(AnimDefaults.tors0*CFrame.Angles(math.rad(35.008), math.rad(0.172), math.rad(-1.547)), i)
		CurrentStand.StandTorso["Left Hip"].C0 = CurrentStand.StandTorso["Left Hip"].C0:Lerp(AnimDefaults.lleg*CFrame.new(0.055, -0.008, -0.009) * CFrame.Angles(math.rad(2.75), math.rad(5.386), math.rad(14.037)), i)
		CurrentStand.StandTorso["Right Hip"].C0 = CurrentStand.StandTorso["Right Hip"].C0:Lerp(AnimDefaults.rleg*CFrame.new(0.977, 0.511, -0.028) * CFrame.Angles(math.rad(-3.037), math.rad(3.151), math.rad(-12.662)), i)
	...
	coroutine.resume(coroutine.create(function()
		wait(0.06)
		CreateHitBox(CurrentStand.PrimaryPart.CFrame * CFrame.new(0, 0, -2.35), "Instant Kill", Vector3.new(4.2,3.4,4), 0.35, true, "Slice", 2974875851, "Strong", Color3.fromRGB(255, 255, 255))
	end))
	for i = 0,1,0.036*AttackSpeed/0.75 do
		game:GetService("RunService").RenderStepped:Wait()
		CurrentStand.RootPart["Root Joint"].C0 = CurrentStand.RootPart["Root Joint"].C0:Lerp(AnimDefaults.tors0*CFrame.Angles(math.rad(33.289), math.rad(-11.631), math.rad(-18.908)), i)
		CurrentStand.StandTorso["Left Hip"].C0 = CurrentStand.StandTorso["Left Hip"].C0:Lerp(AnimDefaults.lleg*CFrame.new(0.056, -0.008, 0.011) * CFrame.Angles(math.rad(11.918), math.rad(17.704), math.rad(12.089)), i)
		CurrentStand.StandTorso["Right Hip"].C0 = CurrentStand.StandTorso["Right Hip"].C0:Lerp(AnimDefaults.rleg*CFrame.new(0.971, 0.512, -0.238) * CFrame.Angles(math.rad(-10.6), math.rad(13.522), math.rad(-11.173)), i)
...
function StandJump()
	...
	for i = 1, 3 do
		...
		CreateEffect(EffectPart, "Slice", Color3.fromRGB(179, 66, 136))
	end
	local Bod = Instance.new("BodyPosition",Root)
	Bod.MaxForce = Vector3.new(math.huge,math.huge,math.huge)
	Bod.Position = Root.CFrame * CFrame.new(0,80,-150).p
	wait(0.1)
	Bod:Destroy()
end
...
function StarFinger()
	...
	wait(0.05)
	CreateSound(1825260752, CurrentStand.Head, 6.2, 1, 0, false, false, "Oofing")
	...
	for i = 0,1,0.105*AttackSpeed/0.75 do
		...
		Arm.Mesh.Scale = Arm.Mesh.Scale:Lerp(Vector3.new(Arm.Mesh.Scale.X, 0.5, Arm.Mesh.Scale.Z), i/2.5)
		Arm.CFrame = Arm.CFrame * CFrame.new(0,-0.605,0)
...
Mouse.KeyDown:Connect(function(Key)
	...
	if Key == "g" then
		if Head:FindFirstChild("Oofio") then
			return
		end
		CreateSound(1825260752, Head, 9.5, 1, 0, false, false, "Oofio")
	end
...
game:GetService("RunService").RenderStepped:Connect(function()
	...
	if Move == true and CurrentStand ~= nil then
		if (BarrageDown == true or StrongOOF == true or NormalPunch == true) and PunchAfterBarrage == false then
...
while wait(0.25) do
	if CurrentStand ~= nil then
		CreateEffect(CurrentStand.StandTorso, "Slice", Color3.fromRGB(245, 205, 48))
	end
end