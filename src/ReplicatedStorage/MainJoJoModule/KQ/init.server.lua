...
script:WaitForChild("SheerHeartAttack")
script:WaitForChild("TouchIndexer")
script:WaitForChild("FeModule")
local OriginalStand = script["Stand"]:Clone()
local OriginalSHA = script["SheerHeartAttack"]:Clone()
local OriginalTouchScript = script["TouchIndexer"]:Clone()
wait(0.25)
script["Stand"]:Destroy()
script["SheerHeartAttack"]:Destroy()

require(script.FeModule)()

local StandName = "Killer Queen"
local ModelCreator = "jojoFMer"
...
local ExplodeTarget = nil
local CurrentTouchConnection = nil
	...
	HB.Touched:Connect(function(Toucher)
		if IsAHumanoid(Toucher) and IsAHumanoid(Toucher) ~= Character and IsAHumanoid(Toucher):FindFirstChildOfClass("Humanoid").Health > 0 and IsAHumanoid(Toucher):FindFirstChild("JustGotHitten") == nil then
			...
			if HitType ~= "Strong" then
				...
				CreatePopUpGui(Toucher, "Jump", tostring(50), Color3.fromRGB(188, 0, 0))
				HitEffect(Toucher, Humanoid.Parent, CurrentStand.RootPart.CFrame, "Punch")
			end
		end
	end)
...
function Stand()
	...
	if CurrentStand == nil then
		...
		CreateSound(2652080817, Head, 5, 1, 0, false, false, "Stand Call")
		...
		for Index,Child in next, CurrentStand:GetDescendants() do
			if (Child:IsA("BasePart") or Child:IsA("Decal")) and Child.Name ~= "RootPart" and Child.Name ~= "FacePart" and Child.Parent ~= CurrentStand then
		...
		CreateSound(3426347398, CurrentStand.PrimaryPart, 4.7, math.random(97,103)/100, 0, false, false, "Stand Appear")
		for i = 0,1,0.02 do
			game:GetService("RunService").RenderStepped:Wait()
			CurrentStand.PrimaryPart.CFrame = CurrentStand.PrimaryPart.CFrame:Lerp(Root.CFrame * CFrame.new(-2, 1.3, 2.5), i)
			CurrentStand.RootPart["Root Joint"].C0 = CurrentStand.RootPart["Root Joint"].C0:Lerp(AnimDefaults.tors0*CFrame.new(0, 0, 0.8*math.sin(Sine/75))*CFrame.Angles(math.rad(-6.016), 0, math.rad(-21.028)), 0.23)
			CurrentStand.StandTorso["Left Hip"].C0 = CurrentStand.StandTorso["Left Hip"].C0:Lerp(AnimDefaults.lleg*CFrame.new(-0.004, 0.04, 0.002) * CFrame.Angles(math.rad(-5.329), math.rad(8.938), math.rad(5.042)), 0.23)
			CurrentStand.StandTorso["Right Hip"].C0 = CurrentStand.StandTorso["Right Hip"].C0:Lerp(AnimDefaults.rleg*CFrame.new(0.04, 0.074, -0.058) * CFrame.Angles(math.rad(-8.193), math.rad(-11.918), math.rad(-5.042)), 0.23)
			CurrentStand.StandTorso["Left Shoulder"].C0 = CurrentStand.StandTorso["Left Shoulder"].C0:Lerp(AnimDefaults.larm*CFrame.new(-0.01, 0.099, 0.004) * CFrame.Angles(math.rad(101.414), math.rad(-20.626), math.rad(-29.794)), 0.23)
			CurrentStand.StandTorso["Right Shoulder"].C0 = CurrentStand.StandTorso["Right Shoulder"].C0:Lerp(AnimDefaults.rarm*CFrame.new(-0.008, 0.218, -0.083) * CFrame.Angles(math.rad(49.045), math.rad(11.058), math.rad(8.193)), 0.23)
			CurrentStand.StandTorso.Neck.C0 = CurrentStand.StandTorso.Neck.C0:Lerp(AnimDefaults.head*CFrame.Angles(math.rad(5.787), math.rad(2.464), math.rad(16.845)), 1)
		end
	else
		for Index,Child in next, CurrentStand:GetDescendants() do
			if Child:IsA("BasePart") or Child:IsA("Decal") then
...
function Barrage(Times, NeutralStop, BarrageSFX, External)
	...
	local Done = false
	local Repeat = Times
	local BarrageSound = CreateSound(BarrageSFX, CurrentStand.Head, 4.25, 1, 0.275, not NeutralStop, not NeutralStop, "Barrage Shout")
	...
	if NeutralStop == false and External == true then
		coroutine.resume(coroutine.create(function()
			while BarrageSound do
				game:GetService("RunService").RenderStepped:Wait()
				if BarrageSound.TimePosition > 4.25 then
					BarrageSound:Pause()
					BarrageSound.TimePosition = 2.1
					BarrageSound:Resume()
				end
			end
		end))
...
function StrongPunch()
	...
	local WalkSpeed = Hum.WalkSpeed
	Hum.WalkSpeed = 7
	CreateSound(2500596035, CurrentStand.Head, 6.5, math.random(97,105)/100, 0.01, false, false, "Strong Shout")
...
function TouchConnector(Object)
	if CurrentTouchConnection ~= nil then
		CurrentTouchConnection:Disconnect()
		if TouchedPart ~= nil then
			local Script = OriginalTouchScript:Clone()
			Script["RemoveIt"].Value = true
			Script.Object.Value = TouchedPart
			Script.Parent = Player:FindFirstChildOfClass("PlayerGui")
			Script.Disabled = false
			game:GetService("Debris"):AddItem(Script, 0.25)
		end
		TouchedPart = nil
	end
	local Script = OriginalTouchScript:Clone()
	Script["RemoveIt"].Value = false
	Script.Object.Value = Object
	Script.Parent = Player:FindFirstChildOfClass("PlayerGui")
	Script.Disabled = false
	game:GetService("Debris"):AddItem(Script, 0.25)
	TouchedPart = Object
	local DidTouched = false
	CurrentTouchConnection = Object.Touched:Connect(function(Toucher)
		if IsAHumanoid(Toucher) and IsAHumanoid(Toucher) ~= Character and Toucher ~= Object.Parent and DidTouched == false then
			...
			DidTouched = true
			CreateSound(2652080817, Head, 5, 1, 0, false, false, "Killer Queen")
			Explode(IsAHumanoid(Toucher))
			local Script = OriginalTouchScript:Clone()
			Script["RemoveIt"].Value = true
			Script.Object.Value = Object
			Script.Parent = Player:FindFirstChildOfClass("PlayerGui")
...
function Touching()
	...
	Hitbox.Size = Vector3.new(2.25, 2.25, 2.25)
	Hitbox.Transparency = 1
	Hitbox.CFrame = Root.CFrame * CFrame.new(1, 0.75, -5.25)
	Hitbox.Name = StandName.."'s HitBox"
	Hitbox.Massless = true
	Hitbox.CanCollide = false
	Hitbox.Anchored = false
	Hitbox.Parent = CurrentStand
	local AnchoredV1 = Instance.new("BodyVelocity")
	AnchoredV1.MaxForce = Vector3.new(math.huge,math.huge,math.huge)
	AnchoredV1.Parent = Hitbox
	AnchoredV1.P = 3e9
	AnchoredV1.Velocity = Vector3.new(Hitbox.CFrame)
	local AnchoredV2 = Instance.new("BodyPosition")
	AnchoredV2.MaxForce = Vector3.new(math.huge,math.huge,math.huge)
	AnchoredV2.Parent = Hitbox
	AnchoredV2.Position = Vector3.new(Hitbox.CFrame)
	AnchoredV2.P = 3e9
	AnchoredV2.D = 3e9
	game:GetService("Debris"):AddItem(Hitbox, 0.5)
	local TempTouchConnection = Hitbox.Touched:Connect(function(Toucher)
		if not Toucher:IsDescendantOf(Character) and not Toucher:IsA("Terrain") and Touched == false then
			Touched = true
			TouchConnector(Toucher)
		end
	end)
	for i = 0,1,0.03925*AttackSpeed/0.75 do
		game:GetService("RunService").RenderStepped:Wait()
		if Hitbox ~= nil then
			Hitbox.CFrame = Root.CFrame * CFrame.new(1, 0.75, -5.25)
		end
	...
	coroutine.resume(coroutine.create(function()
		wait(1)
		TempTouchConnection:Disconnect()
	end))
	StrongPunching = false
	Move = false
end
...
function Explode(Target)
	...
	Move = true
	for i = 0,1,0.06*AttackSpeed/0.75 do
		game:GetService("RunService").RenderStepped:Wait()
		CurrentStand.RootPart["Root Joint"].C0 = CurrentStand.RootPart["Root Joint"].C0:Lerp(AnimDefaults.tors0*CFrame.Angles(math.rad(-6.016), 0, math.rad(-21.028)), i)
		CurrentStand.StandTorso["Left Hip"].C0 = CurrentStand.StandTorso["Left Hip"].C0:Lerp(AnimDefaults.lleg*CFrame.new(-0.004, 0.04, 0.002) * CFrame.Angles(math.rad(-5.329), math.rad(8.938), math.rad(5.042)), i)
		CurrentStand.StandTorso["Right Hip"].C0 = CurrentStand.StandTorso["Right Hip"].C0:Lerp(AnimDefaults.rleg*CFrame.new(0.04, 0.074, -0.058) * CFrame.Angles(math.rad(-8.193), math.rad(-11.918), math.rad(-5.042)), i)
		CurrentStand.StandTorso["Left Shoulder"].C0 = CurrentStand.StandTorso["Left Shoulder"].C0:Lerp(AnimDefaults.larm*CFrame.new(0.002, -0.02, -0.001) * CFrame.Angles(math.rad(-8.365), math.rad(-1.203), math.rad(3.151)), i)
		CurrentStand.StandTorso["Right Shoulder"].C0 = CurrentStand.StandTorso["Right Shoulder"].C0:Lerp(AnimDefaults.rarm*CFrame.new(0.892, 0.5, 0.128) * CFrame.Angles(math.rad(96.429), math.rad(3.438), math.rad(37.586)), i)
		CurrentStand.StandTorso.Neck.C0 = CurrentStand.StandTorso.Neck.C0:Lerp(AnimDefaults.head*CFrame.Angles(math.rad(5.844), math.rad(2.406), math.rad(-1.203)), i)
	end
	CreateSound(920181099, CurrentStand.Head, 8.5, 1, 0, false, false, "KQ Click")
	spawn(function()
		wait(0.45)
		coroutine.resume(coroutine.create(function()
			for Index, Child in next, CurrentTarget:GetChildren() do
				if Child:IsA("BasePart") then
			...
			wait(0.825)
			CurrentTarget:BreakJoints()
		end))
		CreateSound(2686077452, CurrentTarget.Head, 10, 1, 0, false, false, "SHA Explode")
		coroutine.resume(coroutine.create(function()
			wait(1)
			...
			Exp.Position = CurrentTarget.Head.Position
			Exp.Parent = CurrentTarget.Head
			game:GetService("Debris"):AddItem(Exp, 3)
		end))
	end)
	for i = 0,1,0.0525*AttackSpeed/0.75 do
		game:GetService("RunService").RenderStepped:Wait()
		CurrentStand.RootPart["Root Joint"].C0 = CurrentStand.RootPart["Root Joint"].C0:Lerp(AnimDefaults.tors0*CFrame.Angles(math.rad(-6.016), 0, math.rad(-21.028)), i)
		CurrentStand.StandTorso["Left Hip"].C0 = CurrentStand.StandTorso["Left Hip"].C0:Lerp(AnimDefaults.lleg*CFrame.new(-0.004, 0.04, 0.002) * CFrame.Angles(math.rad(-5.329), math.rad(8.938), math.rad(5.042)), i)
		CurrentStand.StandTorso["Right Hip"].C0 = CurrentStand.StandTorso["Right Hip"].C0:Lerp(AnimDefaults.rleg*CFrame.new(0.04, 0.074, -0.058) * CFrame.Angles(math.rad(-8.193), math.rad(-11.918), math.rad(-5.042)), i)
		CurrentStand.StandTorso["Left Shoulder"].C0 = CurrentStand.StandTorso["Left Shoulder"].C0:Lerp(AnimDefaults.larm*CFrame.new(0.002, -0.02, -0.001) * CFrame.Angles(math.rad(-8.365), math.rad(-1.203), math.rad(3.151)), i)
		CurrentStand.StandTorso["Right Shoulder"].C0 = CurrentStand.StandTorso["Right Shoulder"].C0:Lerp(AnimDefaults.rarm*CFrame.new(0.895, 0.433, -0.012) * CFrame.Angles(math.rad(74.485), math.rad(-9.626), math.rad(30.424)), i)
		CurrentStand.StandTorso.Neck.C0 = CurrentStand.StandTorso.Neck.C0:Lerp(AnimDefaults.head*CFrame.Angles(math.rad(14.324), math.rad(5.615), math.rad(-1.776)), i)
	end
	Move = false
end
...
Mouse.KeyDown:Connect(function(Key)
	...
	if Key == "e" and Move == false then
		BarrageDown = true
		Barrage(30, false, 3339464031, true)
	end
	if Key == "t" and Move == false then
		if workspace:FindFirstChild(Player.Name.."'s SHA") then
			...
			if SHA:FindFirstChild("IsExploding") == nil then
				...
				CreateSound(2686077452, ExplosionPart, 10, 1, 0.9725, false, false, "SHA Explode")
