...
--=(Category = Fun, Action, Quick, Memey, Overpowered)=--
...
local StandName = "Flamingo Platinum"
local ModelCreator = "KoolKIDSeventi8"
	...
	HB.Touched:Connect(function(Toucher)
		if IsAHumanoid(Toucher) and IsAHumanoid(Toucher) ~= Character and IsAHumanoid(Toucher):FindFirstChildOfClass("Humanoid").Health > 0 then
			...
			if TimeStopped == false then
				...
				CreateSound(201674629, Toucher, 6.5, math.random(95,113)/100, 0.1, false, false, "Hit SFX")
				...
				table.insert(TimeStopHits, {Humanoid, Damage*2, 201674629, Effect, EffectColor, CurrentStand.RootPart.CFrame, HitType})
			end
		end
	end)
...
function Stand()
	...
	if CurrentStand == nil then
		...
		CreateSound(2762129673, Head, 5, 1, 0, false, false, "Stand Call")
...
function Barrage(Times, NeutralStop, BarrageSFX, External)
	...
	local Done = false
	local Repeat = Times
	local BarrageSound = CreateSound(BarrageSFX, CurrentStand.Head, 5.25, 1, 0.3, not NeutralStop, not NeutralStop, "Barrage Shout")
...
function TimeStop()
	...
	Clock.Enabled = true
	Lock(Character)
	local StopAudio = CreateSound(4554463745, CurrentStand.Head, 8.5, 1, 0, false, false, "Stop Shout")
	repeat
		game:GetService("RunService").RenderStepped:Wait()
	...
	until StopAudio.TimePosition > 0.7
	TimeStopped = true
	StopAudio:Destroy()
...
function StrongPunch()
	...
	local WalkSpeed = Hum.WalkSpeed
	Hum.WalkSpeed = 7
	CreateSound(722744682, CurrentStand.Head, 6.5, math.random(97,108)/100, 0, false, false, "Strong Shout")
	for i = 0,1,0.36*AttackSpeed/0.75 do
		game:GetService("RunService").RenderStepped:Wait()
		CurrentStand.RootPart["Root Joint"].C0 = CurrentStand.RootPart["Root Joint"].C0:Lerp(AnimDefaults.tors0*CFrame.Angles(math.rad(35.008), math.rad(0.172), math.rad(-1.547)), i)
		CurrentStand.StandTorso["Left Hip"].C0 = CurrentStand.StandTorso["Left Hip"].C0:Lerp(AnimDefaults.lleg*CFrame.new(0.055, -0.008, -0.009) * CFrame.Angles(math.rad(2.75), math.rad(5.386), math.rad(14.037)), i)
		CurrentStand.StandTorso["Right Hip"].C0 = CurrentStand.StandTorso["Right Hip"].C0:Lerp(AnimDefaults.rleg*CFrame.new(0.977, 0.511, -0.028) * CFrame.Angles(math.rad(-3.037), math.rad(3.151), math.rad(-12.662)), i)
	...
	for i = 0,1,0.37*AttackSpeed/0.75 do
		game:GetService("RunService").RenderStepped:Wait()
		CurrentStand.RootPart["Root Joint"].C0 = CurrentStand.RootPart["Root Joint"].C0:Lerp(AnimDefaults.tors0*CFrame.Angles(math.rad(35.008), math.rad(0.172), math.rad(-1.547)), i)
		CurrentStand.StandTorso["Left Hip"].C0 = CurrentStand.StandTorso["Left Hip"].C0:Lerp(AnimDefaults.lleg*CFrame.new(0.055, -0.008, -0.009) * CFrame.Angles(math.rad(2.75), math.rad(5.386), math.rad(14.037)), i)
		CurrentStand.StandTorso["Right Hip"].C0 = CurrentStand.StandTorso["Right Hip"].C0:Lerp(AnimDefaults.rleg*CFrame.new(0.977, 0.511, -0.028) * CFrame.Angles(math.rad(-3.037), math.rad(3.151), math.rad(-12.662)), i)
...
function StarFinger()
	...
	wait(0.05)
	local Scream = CreateSound(1251737869, CurrentStand.Head, 5.2, 1, 6.5, false, false, "Albert busts your ears")
	coroutine.resume(coroutine.create(function()
		wait(0.7)
		Scream:Destroy()
	end))
	...
	local HB = CreateHitBox(Arm.CFrame, math.random(50,70), Arm.Size+Vector3.new(3,6,3), 0.5, true, "Slice", 2974875851, "Strong", Color3.fromRGB(255, 102, 204))
	Arm.Transparency = 0
...
Mouse.KeyDown:Connect(function(Key)
	...
	if Key == "g" then
		if Head:FindFirstChild("Not Fair") then
			return
		end
		CreateSound(3535031732, Head, 5.5, 1, 0, false, false, "Not Fair")
	end
...
game:GetService("RunService").RenderStepped:Connect(function()
	...
	if Move == true and CurrentStand ~= nil then
		if (BarrageDown == true or StrongAAA == true or NormalPunch == true) and PunchAfterBarrage == false then
...
while wait(0.25) do
	if CurrentStand ~= nil then
		CreateEffect(CurrentStand.StandTorso, "Slice", Color3.fromRGB(179, 66, 136))
	end
end